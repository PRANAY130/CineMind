...
def run_video_pipeline(self, video_id: int, r2_url: str):
    ...
    try:
        ...
        print(f"[Pipeline] Failed to connect to DB: {e}")
    ...
    def update_progress(step: str, pct: int):
        self.update_state(state='PROGRESS', meta={'step': step, 'progress_pct': pct})
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE jobs SET step = %s, progress_pct = %s, updated_at = CURRENT_TIMESTAMP WHERE video_id = %s",
                    (step, pct, video_id)
                )
                if cur.rowcount == 0:
                    cur.execute(
                        "INSERT INTO jobs (video_id, step, progress_pct) VALUES (%s, %s, %s)",
                        (video_id, step, pct)
                    )
            conn.commit()
        except Exception as e:
            print(f"[Pipeline] DB progress error: {e}")
            conn.rollback()
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            r2_key = parsed.path.lstrip('/').split('/', 1)[1]
            ...
            if HAS_FFMPEG:
                try:
                    ...
                    if probe.returncode == 0:
                        ...
                        with conn.cursor() as cur:
                            cur.execute(
                                "UPDATE videos SET duration_sec = %s WHERE id = %s",
                                (duration, video_id)
                            )
                        conn.commit()
                except Exception as e:
            ...
            if HAS_FFMPEG:
                ...
                try:
                    ...
                    update_progress('Transcribing with Whisper...', 35)
            ...
            update_progress('Indexing transcript...', 55)
            try:
            ...
            update_progress('Generating chapters...', 70)
            chapters = []
            ...
            update_progress('Saving chapters...', 85)
